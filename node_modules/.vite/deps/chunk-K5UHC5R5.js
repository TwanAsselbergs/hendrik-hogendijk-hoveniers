...
function ns_template(content, flags, ns = "svg") {
  var is_fragment = (flags & TEMPLATE_FRAGMENT) !== 0;
  var fn = template(`<${ns}>${content}</${ns}>`, 0);
  var node;
  return () => {
    if (hydrating) {
      push_template_node(is_fragment ? hydrate_nodes : hydrate_start);
        ...
        if (!delegated) {
          if (!prev) {
            events.push([
              key2,
              value,
              () => next[key2] = create_event(event_name, element2, value, opts)
            ]);
          } else {
            next[key2] = create_event(event_name, element2, value, opts);
          }
...
//# sourceMappingURL=chunk-K5UHC5R5.js.map
